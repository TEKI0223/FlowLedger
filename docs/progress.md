...
已实现：信用卡 schema 新增 cycle_boundary 字段，按 inclusive / exclusive 划分账单周期；查询模型动态算账单（不存账单表）；/credit-cards 列表 + /credit-cards/[id] 详情（当期 + 历史 5 期，逾期 / 已还 / 待还徽标）；/credit-cards/[id]/repay 复用 TransactionForm 预填还款转账；首页 sidebar 新增"信用卡账单"入口。还款关联用启发式：occurredOn ∈ (上期 dueDate, 本期 dueDate] 的转账归本期。
...
| M5 余额型账户、现金和校准 | 进行中 | 调整交易语义改造、账户详情页、钱包 / 现金快捷入口               |
...
- ~~调整交易当前实现为"输入差额"~~ 已在 M5 改造：新建调整交易（mode=create）使用"输入实际余额，系统算差额"UI；编辑已有调整保留差额输入（向后兼容）。
- ~~每个支付方式应有默认资金来源账户字段（需求 §4.4）~~ 已在 M3 接入：交易表单和周期项表单选支付方式时自动把 `payment_methods.defaultAccountId` 填到来源账户，用户可覆盖。
