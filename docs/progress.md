...
M5 已收尾。调整交易改为「输入实际余额」、账户详情页（余额 / 本月流入流出 / 最近交易 / 待处理周期项）、钱包/现金/校准/编辑快捷入口、删除确认 AlertDialog、通用 MoneyInput（千分位）、表单失败状态保留、校准精度错误内联显示均通过浏览器验收。
